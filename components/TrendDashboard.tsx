...
function buildMethodologyHrefFromLocation(): string {
  if (typeof window === "undefined") {
    return "/methodology";
  }
  const source = `${window.location.pathname}${window.location.search}${window.location.hash}`;
  const from = source === "/methodology" || source.startsWith("/methodology?") ? "/" : source;
  return `/methodology?from=${encodeURIComponent(from || "/")}`;
}

function buildDashboardHrefFromLocation(): string {
  if (typeof window === "undefined") {
    return "/";
  }
  const source = `${window.location.pathname}${window.location.search}${window.location.hash}`;
  if (!source || source === "/methodology" || source.startsWith("/methodology?")) {
    return "/";
  }
  return source;
}
...
function TrendApp({
  ...
  const [methodologyHref, setMethodologyHref] = useState("/methodology");
  ...
  const loadPair = useCallback(async (pair: string, options: { warmAi?: boolean; updateUrl?: boolean } = {}) => {
    ...
    try {
      ...
      if (options.updateUrl) {
        ...
        setDashboardHref(buildDashboardHrefFromLocation());
        setMethodologyHref(buildMethodologyHrefFromLocation());
  ...
  return (
    <main className="page-shell">
      <SiteHeader
        ...
        latestData={relationship?.data_end ?? null}
        locale={locale}
...
function SiteFooter({ copy, methodologyHref }: { copy: DashboardCopy; methodologyHref: string }) {
  const links = [
    { href: "/", label: copy.footer.home },
    { href: "/about", label: copy.footer.about },
    { href: methodologyHref, label: copy.footer.methodology },
